<template>
  <div
    class="compare-visas"
    :data-total="compareData.length"
  >
    <span
      v-if="compareData"
      v-show="compareData.length > 4"
      class="compare-visas__shadow"
    ></span>
    <div class="compare-visas__container">
      <div class="country-row">
        <div
          ref="list"
          class="country-row__container country-row__container--titles"
        >
          <ul class="country-cols country-cols--titles">
            <li class="country-cols__column country-cols__column--title">
              <div
                v-if="compareData.length > 1"
                class="country-row__tools"
              >
            ...
            >
              ...
              <div class="labels">
                <visa-label
                  type="total"
                  :show-text="false"
                  :count="mergedData.score.total"
                />
                <visa-label
                  type="free"
                  :show-text="false"
                  :count="mergedData.score.free"
                />
                <visa-label
                  type="eta"
                  :show-text="false"
                  :count="mergedData.score.eta"
                />
                <visa-label
                  type="arrival"
                  :show-text="false"
                  :count="mergedData.score.arrival"
                />
                <visa-label
                  type="required"
                  :show-text="false"
                  :count="mergedData.score.required"
                />
              </div>
            </li>
            <li
              v-for="(country, index) in compareData"
              :key="index"
              class="country-cols__column"
            >
              <h4>
                <span
                  class="icon remove"
          ...
          <ul class="country-cols country-cols--countries">
            <li class="country-cols__column country-cols__column--title">
              <ul class="country-cols__list">
                <li
                  v-for="(c, i) in blacklistFilter(countryList, false, 'titles')"
                  :key="i"
                  class="country-cols__item"
                >
                  <h5 class="sub">
                    <span
                      class="long_title"
            ...
            >
              <ul class="country-cols__list">
                <li
                  v-for="(c, i) in blacklistFilter(mergedData.data, true, 'merged')"
                  :key="i"
                  class="country-cols__item"
                >
            ...
            >
              <ul class="country-cols__list">
                <li
                  v-for="(c, i) in blacklistFilter(country.data, true, 'list')"
                  :key="i"
                  class="country-cols__item"
                >
                  <visa-label :type-number="c" />
      ...
      if (_this.differences) {
        if (!onlyData) {
          Object.keys(data).forEach(item => {
            if (
              !_this.blacklist[data[item].id] &&
              _this.blacklist[data[item].id] !== 0
            ) {
    ...
    createCompareData() {
      ...
      if (this.$props.countries.length > 0) {
        this.$props.countries.forEach(country => {
          data.push(
            this.$store.state.passport.countryList.find(o => o.id === country)
          );
        });
      } else {
...
.icon.remove {
  ...
  @media #{$small-only} {
    ...
    transform: translate(-50%, -100%) rotate(45deg) scale(1);
    &:hover {
      transform: translate(-50%, -100%) rotate(-45deg) scale(1.5);
    }
  }
}
